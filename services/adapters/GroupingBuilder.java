...
public class GroupingBuilder extends Neo4jDbConnection {
    ...
    public GroupingBuilder(String query, HashMap<String, Color> colors) {
        super();
        try {
            boolean flag = false;
            String temp = "";
            ...
            for (int i = 0; i < query.length(); i++) {
                if (query.charAt(i) == ']') {
                    ...
                    if (temp.isEmpty() || temp.contains("INN") && temp.contains("ROUTES"))
                        q = q + "[:INN|ROUTES*]";
                    else {
                        if (temp.contains("INN"))
                            q = q + "[:INN*]";
                        else if (temp.contains("ROUTES"))
                            q = q + "[:ROUTES*]";
                        else {
                            new MistakeQuery();
                            return;
            ...
            try(Transaction tx = this.graphDb.beginTx();
                ...
                drawGraph(tx, new GraphDraw("Network Grouping", colors));
