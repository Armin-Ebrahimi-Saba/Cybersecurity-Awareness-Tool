...
import services.library.drowTools.FilteringGraphDraw;
...
public class QueryBuilder extends Neo4jDbConnection {
    ...
    public QueryBuilder(String q, String graphDrawTitle, HashMap<String, Color> colors){
        super();
        this.colors = colors;
        ...
        if (Query.findByQuery(q) == null){
            Query queryRecord = new Query(q);
            queryRecord.save();
        }
        ...
        try {
            ...
            try(Transaction tx = this.graphDb.beginTx();
                ...
                drawGraph(tx, new GraphDraw(graphDrawTitle,colors));
                if (graphDrawTitle.equals("CyGraph") || graphDrawTitle.equals("Filtering"))
                    new AnalyzeLayout(q,colors);
...
/*
    public QueryBuilder(String q, HashMap<String, FilterPanel> filterPanels){
        super();
        if (q.isEmpty() || q.isBlank()){
            return;
        }
        ...
        try {
            ...
            try(Transaction tx = this.graphDb.beginTx();
                ...
                drawGraph(tx, new FilteringGraphDraw("Filtering",filterPanels));
    ...
    private void drawGraph(Transaction tx, FilteringGraphDraw frame) {
        if (queue.isEmpty()) {
            return;
        }
