...
import services.library.drowTools.LayeringGraphDraw;
import services.library.drowTools.NodeHelper;

import java.awt.*;
import java.util.*;

public class LayeringBuilder extends Neo4jDbConnection {
    ...
    public LayeringBuilder(String query, HashMap<String, Color> colors) {
        super();
        try {
            ...
            try (Transaction tx = this.graphDb.beginTx();
                            ...
                            for (Object id : (ArrayList)column.getValue()) {
                                ...
                                switch (properties.get("collection").toString()) {
                                    case "attack_patterns":
                                        if (nodeLayers.containsKey("attack_patterns")) {
                                            if (!nodeLayers.get("attack_patterns").contains(id))
                                                nodeLayers.get("attack_patterns").add((Long) id);
                                        }
                                        else {
                                            item.add((long)id);
                                            nodeLayers.put("attack_patterns",item);
                                        }
                                        break;
                                    case "machines":
                                    ...
                                    case "firewalls":
                                        ...
                                        break;
                                    default:
                                        if (nodeLayers.containsKey("others")) {
                                            if (!nodeLayers.get("others").contains(id))
                                                nodeLayers.get("others").add((Long) id);
                                        }
                                        else {
                                            item.add((long)id);
                                            nodeLayers.put("others",item);
                                        }
                                }
                                indexNodeId = (long)id;
                ...
                drawGraph(tx, new LayeringGraphDraw("Layering", colors));
    ...
    private void drawGraph(Transaction tx, LayeringGraphDraw frame) {
        if (queue.isEmpty()) {
            return;
        }
        ...
        switch (properties.get("collection").toString()) {
            case "attack_patterns":
                parentNodeHelper.y = 100;
                m = nodeLayers.get("attack_patterns").size();
                parentNodeHelper.x = 1000 -  ((size_layers.get("attack_patterns") - 0.5) / m) * 1000;
                size_layers.put("attack_patterns", size_layers.get("attack_patterns") - 1);
                break;
            case "machines":
                parentNodeHelper.y = 200;
                m = nodeLayers.get("machines").size();
                parentNodeHelper.x = 1000 -  ((size_layers.get("machines") - 0.5) / m) * 1000;
                size_layers.put("machines", size_layers.get("machines") - 1);
                break;
            case "subnets":
                parentNodeHelper.y = 300;
                m = nodeLayers.get("subnets").size();
                parentNodeHelper.x = 1000 -  ((size_layers.get("subnets") - 0.5) / m) * 1000;
                size_layers.put("subnets", size_layers.get("subnets") - 1);
                break;
            case "firewalls":
                parentNodeHelper.y = 400;
                m = nodeLayers.get("firewalls").size();
                parentNodeHelper.x = 1000 -  ((size_layers.get("firewalls") - 0.5) / m) * 1000;
                size_layers.put("firewalls", size_layers.get("firewalls") - 1);
                break;
            default:
                parentNodeHelper.y = 500;
                m = nodeLayers.get("others").size();
                parentNodeHelper.x = 1000 -  ((size_layers.get("others") - 0.5) / m) * 1000;
                size_layers.put("others", size_layers.get("others") - 1);
        }
